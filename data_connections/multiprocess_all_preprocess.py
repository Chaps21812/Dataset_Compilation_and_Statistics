...
from src.coco_tools import COCODataset, CalsatDataset
import shutil
import os
from src.preprocess_functions import raw_file_16bit, raw_file,iqr_log,iqr_log_16bit
from src.multiprocessing import SimpleFileLogger
...
def preprocess_large_dataset(telescope_directory):
    ...
    file_logger = SimpleFileLogger(os.path.join("/data/Dataset_Compilation_and_Statistics/logs",f'reprocess-{os.path.basename(telescope_directory)}.log'))
...
def remake_TTV_split_and_preprocess(telescope_directory):
    # Create logger object
    file_logger = SimpleFileLogger(os.path.join("/data/Dataset_Compilation_and_Statistics/logs",f'preprocess_raw16bit-{os.path.basename(telescope_directory)}.log'))
    sys.stdout = file_logger
    sys.stderr = file_logger
    print(f"PRocessing Dataset {os.path.basename(telescope_directory)}...(PID={os.getpid()})")
...
def preprocess_correct_large_dataset(telescope_directory):
    # Create logger object
    file_logger = SimpleFileLogger(os.path.join("/data/Dataset_Compilation_and_Statistics/logs",f'sidereal_download-{os.path.basename(telescope_directory)}.log'))
    sys.stdout = file_logger
    sys.stderr = file_logger
    print(f"PRocessing Dataset {os.path.basename(telescope_directory)}...(PID={os.getpid()})")
...
if __name__ == '__main__':
    ...
    tasks = [mp.Process(target=remake_TTV_split_and_preprocess, args=(dataset,)) for dataset in ALL_TELESCOPES]
